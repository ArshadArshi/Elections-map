  ...
  return (
    <div>
         <Card className="w-[400px] p-1">
            <div className="flex justify-around items-center ml-10"><CardTitle>PARTY</CardTitle>
      <CardHeader>
        ...
        {partyData.map(party=>(
        <div>
       <div className="flex justify-around items-center"> <div className="flex justify-center items-center gap-1"> <img className="w-4 h-4" src={party.symbol} alt="BJP" /> | {party.party} </div>
       <div className="flex gap-9"> <div>{party.leads}</div> | <div>{party.win}</div> | <div>{party.total}</div> </div></div>
        </div>
        ))}
        <div className="flex justify-end text-xs pr-7 pt-2">Total Seats : 90/90 </div>
